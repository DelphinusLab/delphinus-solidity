...
function encode_l1address(address_hexstr, chex) {
  let c = new BigNumber(
    chex + "0000000000000000000000000000000000000000",
    "hex"
  );
...
function extract_chain_info(all_tokens) {
  ...
  valid_tokens = valid_tokens.map((token) => {
    ...
    return {
      ...
      index: all_tokens.findIndex((x) => x.token_uid == token.token_uid),
      address: address,
    };
  });
...
class Bridge {
  ...
  getTokenInfo(idx) {
    ...
    return {
      ...
      tokenName:
        Tokens.tokenInfo.find(
          (x) => hexcmp(x.address, addr) && x.chainId == cid
        )?.name || "unknown",
    };
  }
  ...
  deposit(token_address, amount, l2account) {
    ...
    let r = pbinder.return(async () => {
      ...
      let token = Client.getContractByAddress(
        this.web3,
        token_address,
        ERC20,
        this.account
